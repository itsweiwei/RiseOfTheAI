...
void Initialize() {
	...
	state.player.textureID = LoadTexture("luigi.png");

	GLuint evilTextureID = LoadTexture("goomba.png");
	state.enemies[0].entityType = ENEMY;
	state.enemies[0].aiType = WALKER;
	state.enemies[0].textureID = evilTextureID;
	state.enemies[0].isStatic = false;
	state.enemies[0].position = glm::vec3(3, 2.75f, 0);
	state.enemies[0].acceleration = glm::vec3(0, -12.0f, 0);
	...
	state.enemies[1].acceleration = glm::vec3(0, -12.0f, 0);
	state.enemies[1].position = glm::vec3(3, 3.75f, 0);
	state.enemies[1].aiState = IDLE;

	GLuint flyingID = LoadTexture("koopa.png");
	state.enemies[2].entityType = ENEMY;
	state.enemies[2].aiType = FLYER;
	state.enemies[2].textureID = flyingID;
	state.enemies[2].isStatic = false;
	state.enemies[2].collidedBottom = true;
	state.enemies[2].position = glm::vec3(0, 3.75f, 0);
...
void Render() {
	...
	for (int i = 0; i < ENEMY_COUNT; i++) {
		if (state.enemies[i].aiState != DEAD) {
			state.enemies[i].Render(&program);
		}
	}

	if (state.player.isActive == false) {
		DrawText(&program, LoadTexture("alpha.png"), "Game Over", 1.0f, 0.05f, glm::vec3(-4.75f, 0, 0));
	}
	...
	if (killed == 3) {
		DrawText(&program, LoadTexture("alpha.png"), "You Win!", 1.0f, 0.05f, glm::vec3(-4.75f, 0, 0));
	}
